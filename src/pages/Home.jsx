...
export function Home() {
  ...
  function handleSubmit(e) {
    ...
    createData({ url, body })
      .then(data => {
        const { data: dataResponse, token, id_sesion: idSesion } = data
  ...
  return (
    <main className="flex min-h-svh h-svh max-h-svh overflow-clip lgMax:bg-[url(/src/assets/img/businessMan.jpeg)] bg-no-repeat bg-cover">
      <section className="hidden lg:block h-full flex-1">
        <img
      ...
      <section className="relative flex flex-col justify-center items-center h-full p-[100px] lgMax:backdrop-blur-xl bg-[rgba(177,177,177)] bg-opacity-70 lg:bg-white w-full lg:w-[568px]">
        <header className="absolute top-10 right-10 w-[215px] h-[55px]">
          <img className="w-full h-full" src="/LogoBuenosVientos.jpeg" />
        </header>

        <span className="w-full mb-2 text-base font-medium">Supervisor</span>
        ...
        >
          <header className="flex flex-col justify-between gap-5">
            <h1 className="titulo-h4 text-azul-500">Ingrese a su sesión</h1>
            <span className="texto-regular-m text-textoPrincipal">
          ...
          <fieldset className="flex flex-col gap-5">
            ...
            <fieldset className="flex justify-between items-center gap-2">
              <label className="flex gap-2 h-4 items-center w-max cursor-pointer">
                <NormalCheck
                ...
                <span className="text-[#737373] texto-regular-s">
                  Recordar contraseña
                </span>
              </label>
