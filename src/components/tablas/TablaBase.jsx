  ...
  return (
    <section
      className={`flex flex-col gap-2 ${
        rounded ? 'rounded-tablas' : ''
      } ${className}`}
    >
      <section
        className={`w-full ${
          overflow ? 'overflow-auto scroll-neutral' : 'overflow-clip'
        } ${
          rounded ? 'rounded-tr-tablas rounded-tl-tablas' : ''
        } ${classContainer}`}
      >
        <section
          className={`flex flex-col [&_li]:truncate ${
            overflow ? 'w-full min-w-max' : 'w-full max-w-full'
          } gap-2 ${classTable}`}
        >
          <header>
            <ListStyle className="py-3 px-6 text-xs font-normal leading-4 flex-wrap w-full min-h-11">
              {headers.map((header, idx) => (
                <li
                  key={header}
                  title={header}
                  className={`min-w-fit truncate text-nowrap ${
                    headersClassName.length ? headersClassName[idx] : ''
                  }`}
                >
                  {header}
                </li>
          ...
          {loading && <ListStyleRow>Cargando...</ListStyleRow>}
