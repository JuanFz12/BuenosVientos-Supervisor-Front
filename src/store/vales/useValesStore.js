...
export const useValesStore = create(set => {
  function getVales() {
    ...
    return getData({ url })
      .then(dataRes => {
        const { payload: vales } = dataRes.data
      ...
      .catch(err => {
        alert(
          `Error en vales: ${err.error ?? err.message ?? 'Error desconocido'}`
        )
      })
      .finally(() => {
        set({ loading: false })
  ...
  function getSolicitudes() {
    ...
    return getData({ url })
      .then(dataRes => {
        const { payload: solicitudes } = dataRes.data
      ...
      createData({ url, body })
        .then(data => {
          ...
          set(state => {
            ...
            valeAModificar.requestVale.application_status =
              valeAceptado.requestVale.application_status

            return {
              solicitudes: newSolicitudes,
              vales: [valeAceptado, ...state.vales]
            }
          })
  ...
  function crearVale({ body }) {
    return new Promise((resolve, reject) => {
      ...
      createData({ url, body })
        ...
        .catch(err => {
          alert(
            `Error al crear un vale: ${
              err.error ?? err.reason ?? err.message ?? 'Error desconocido'
            }`
          )
          console.warn(err)
          reject(err)
  ...
  function updateEstadoVale({ idRequestVale, status }) {
    ...
    const url = generateUrl(apiRequest.actualizarVale, {
      [paramId]: idRequestVale
    })
    ...
    return new Promise((resolve, reject) => {
      ...
      updateData({ url, body })
        .then(dataRes => {
          ...
          const { data: valeActualizado } = dataRes

          set(state => {
            const index = state.vales.findIndex(
              ({ requestVale: { id } }) => id === idRequestVale
            )
            ...
            valeAModificar.requestVale.application_status =
              valeActualizado.requestVale.application_status
        ...
        .catch(err => {
          alert(
            `Error al actualizar el estado del vale: ${
